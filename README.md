...
4. **Cross-encoder reranking** (Jina v2 multilingual) for final precision. **The cross-encoder is a local ONNX discriminative scorer, not a remote LLM**, it runs on CPU in ~50 ms per query, no API call, no network round trip, no API key required. **Enabled by default** in the public package, but the lite mode without it (`--mode hybrid`) already lands at **98.30% R@5** on LongMemEval, well above any other public memory system's no-LLM number. The cross-encoder pushes the canonical configuration up to **99.15% R@5**, it is the cherry on top, not a hard requirement to be competitive. Disable with `MNEMOS_ENABLE_RERANK=0` or `Mnemos(enable_rerank=False)` if you want the smallest possible RAM footprint; you trade about 0.85 percentage points of R@5 for ~500 MB less resident memory.
...
For any active memory system the bottleneck eventually becomes context budget, not raw retrieval recall. Combining CML with the reranker gives you the best of both: SOTA accuracy and a fraction of the tokens. The full per-mode benchmark numbers live in [`benchmarks/`](benchmarks/).
...
> **Planned**: a unified `MNEMOS_CML_MODE` environment variable that flips all the CML-related surfaces in one place, the MCP tool description (so the agent stops being instructed in CML conventions), the Nyx cycle phase prompts (so consolidation outputs prose instead of cemelifying it), the dedup subject-matching path (falls back to keyword/topic matching), and the briefing/digest renderers (render in prose). Today the opt-out path is the manual one above; the unified switch will make it a single coordinated flag. The benchmark numbers already justify it as a first-class deployment option (the BM25+vector lite mode at **98.30% R@5** is above any other public memory system's no-LLM number, with no CML at all in the loop), so a coherent prose-only deployment is a real product, not a degraded mode.
...
**98.94% R@5 with no asterisk of any kind.** It is also higher than any other published memory system's clean retrieval-recall number on LongMemEval. The closest published competitor with no LLM in the retrieval path is MemPalace raw ChromaDB at 96.6%; everything else in the field either publishes only LLM-judge QA accuracy (a different metric class that bundles retrieval and generation) or relies on a generative LLM in their highest-scoring configurations.
...
If you disagree with whether LLM-based CML preprocessing of the benchmark data counts as fair, **use the `--mode hybrid+rerank` table above instead**. That number (98.94% R@5) is the cleanest possible measurement and still beats every published competitor at every comparable k. The CML number is a more accurate picture of how Mnemos actually performs in steady-state production use; the verbose number is a more conservative measurement of pure retrieval quality. Both are honest, both come from first-run no-tuning runs, both are best in class against every published competitor at every comparable k, and both result files are in `benchmarks/` with timestamps you can verify against the file modification times.
...
LongMemEval's primary metric in the original paper is end-to-end QA accuracy: retrieval returns top-K sessions, an answerer LLM produces an answer from those sessions, and a judge LLM scores the answer against the reference. The 500 questions include 30 abstention cases where the correct behavior is refusal. Published numbers from Mastra (~86%), Emergence (~95%), Mem0, Supermemory, and most of the field are in this metric class.
...
- **What the Sonnet+raw 77.4% number means:** Mnemos's retrieval paired with Claude Sonnet as answerer, using raw retrieved session text as answer context. This is what a Claude Code user pairing Mnemos with Sonnet would actually see. Comparable in spirit to Mastra's ~86% only if both systems use the same answerer. Mastra publishes their pipeline paired with their chosen answerer, so exact apples-to-apples requires a re-run with a standardized answerer (planned).
...
The combined picture: **Mnemos is the only memory system in this comparison with (a) state-of-the-art retrieval recall on LongMemEval, (b) end-to-end QA numbers published alongside retrieval numbers rather than in place of them, (c) fully local CPU-only operation with no generative LLM in the search path, and (d) a stable four-tool MCP surface under 1.5 K tokens of system-prompt overhead.** Other systems may match it on one or two of those axes; based on what they publish, I have not found one that matches on all four.
