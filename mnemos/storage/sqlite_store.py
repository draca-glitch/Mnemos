...
class SQLiteStore(MnemosStore):
    ...
    def init_schema(self):
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS memories (
                ...
                dream_processed TEXT DEFAULT NULL
            )
        """)
        # Indexes
        for idx_sql in [
        ...
        # Dream insights (consolidation history)
        conn.execute("""
        ...
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_dream_memory ON dream_insights(memory_id)")
        conn.commit()
    ...
    def store_dream_insight(self, memory_id, source_ids, insight_type, consolidation_type="aggregation"):
        conn = self._get_conn()
        conn.execute(
            "INSERT INTO dream_insights (memory_id, source_ids, insight_type, consolidation_type) "
            "VALUES (?, ?, ?, ?)",
    ...
    def get_merged_sources(self, memory_id, valid_only=True):
        ...
        conn = self._get_conn()
        row = conn.execute(
            "SELECT source_ids, consolidation_type FROM dream_insights WHERE memory_id = ?",
            (memory_id,),
        ).fetchone()
        if not row:
            return []
