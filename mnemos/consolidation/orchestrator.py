"""
Dream cycle orchestrator for Mnemos.

Runs the 5-phase consolidation pipeline. Phase 5 (Bookkeeping) always runs
...
Usage:
    from mnemos.consolidation import run_dream_cycle
    from mnemos.storage.sqlite_store import SQLiteStore

    store = SQLiteStore()
    stats = run_dream_cycle(store, execute=True)
"""
...
def _migrate_dream_schema(conn):
    """Add dream cycle tables. Safe to run repeatedly."""
    conn.execute("""
...
# --- Phase 5: Bookkeeping (no LLM required) ---
...
def run_dream_cycle(
    ...
    Args:
        ...
        project: Filter Phase 1 to this project
    ...
    if hasattr(store, "_get_conn"):
        ...
        raise ValueError(
            "run_dream_cycle requires a backend with a SQLite connection. "
            "SQLiteStore or QdrantStore are supported)."
        )

    conn.row_factory = sqlite3.Row
    _migrate_dream_schema(conn)

    if phases is None:
        phases = {0, 1, 2, 3, 5}  # default: consolidation, no synthesis

    has_llm = llm_is_configured()
    llm_phases = {1, 2, 3, 4}  # phases that need an LLM

    if not has_llm and (phases & llm_phases):
        log("No LLM configured (MNEMOS_LLM_API_KEY), skipping LLM-dependent phases.")
        log(f"  Skipped phases: {sorted(phases & llm_phases)}")
        log("  Phase 5 (Bookkeeping) will still run.")
        phases = phases - llm_phases

    mode = "EXECUTE" if execute else "DRY RUN"
    log(f"Mnemos Dream Cycle starting ({mode}, phases={sorted(phases)})")
    ...
    if phases & llm_phases:
        ...
        if len(all_embeddings) < 2:
            ...
            if 1 in phases:
                phase_stats["phase1"] = phase_dedup(
                ...
                if execute:
                    ...
                    if (p1.get("tight_merged", 0) + p1.get("topic_merged", 0)) > 0:
                        ...
                        all_embeddings, mergeable_embeddings, mem_by_id = load_embeddings(conn)

            if 2 in phases:
                phase_stats["phase2"] = phase_weave(
                    conn, all_embeddings, mem_by_id, is_surge, execute=execute
                )

            if 3 in phases:
                phase_stats["phase3"] = phase_contradict(
                    conn, mergeable_embeddings, mem_by_id, is_surge, execute=execute
                )
            ...
            if 4 in phases:
                ...
                if not is_surge and last_run:
                    ...
                    if len(new_originals) < 5:
                        log(f"Phase 4: Skipping synthesis ({len(new_originals)} new originals, need 5+)")
                        should_synth = False
                if should_synth:
                    phase_stats["phase4"] = phase_synthesize(
                        conn, all_embeddings, mem_by_id, execute=execute
                    )

    # --- Phase 5: Bookkeeping (always runs) ---
    if 5 in phases:
        log("Phase 5: Bookkeeping...")
    ...
    log(f"Dream cycle complete in {elapsed:.1f}s")
        ...
        try:
            conn.execute(
                ...
                (
                    ...
                    p1.get("archived", 0),
                    p1.get("created", 0),
                    f"mnemos-dream-cycle, phases={sorted(phases)}",
