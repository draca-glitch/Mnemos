"""
Dream Cycle phases for memory consolidation v2.

Five phases modeled after brain sleep consolidation:
  Phase 0: Triage. classify new memories, estimate budget
  Phase 1: Dedup. two-tier merge (tight + topic)
  Phase 2: Weave. cross-category connection detection
  Phase 3: Contradict. temporal evolution detection
  Phase 4: Synthesize. generate novel cross-domain insights
  Phase 5: Bookkeep. decay, cleanup, logging (in main script)
...
def fastembed_embed(texts, prefix="passage"):
    """Backwards-compat adapter for the dream phase code."""
...
def _is_dream_generated(mem):
    """Check if a memory was generated by the dream cycle."""
    tags = (mem.get("tags") or "").lower()
    return any(t in tags for t in ("synthesized", "dream-cycle", "bridge"))
...
def find_clusters(ids, sim_matrix, threshold, mem_by_id, max_clusters=50,
    ...
    for _, seed_i, seed_j in pairs:
        ...
        if len(cluster) >= 2:
            ...
            # Skip clusters that are entirely dream-generated or consolidated (anti-churn)
            if mem_by_id:
                has_original = any(
                    not _is_dream_generated(mem_by_id.get(mid, {}))
...
def merge_cluster(cluster_ids, mem_by_id):
    ...
    for mid in cluster_ids:
        ...
        parts.append(
            f"--- Memory #{mid} [{mem['project']}] ({created}) "
            f"tags:{mem.get('tags', '')} imp:{mem.get('importance', 5)} ---\n"
            f"{mem['content']}"
        )

    memories_text = "\n\n".join(parts)
    prompt = f"Merge these {len(cluster_ids)} related memories into one CML block:\n\n{memories_text}"
...
def apply_merge(conn, cluster_ids, merged_content, mem_by_id):
    ...
    # Embed the merged memory
    try:
        text = prep_memory_text(project, merged_content,
                                ",".join(sorted(all_tags)), mem_type=inherit_type)
...
def phase_triage(conn, mem_by_id, last_run_at):
    ...
    log(f"Phase 0: {len(new_ids)} new memories since last run"
...
def phase_dedup(conn, mergeable_embeddings, mem_by_id, is_surge, execute=False):
    ...
    if len(mergeable_embeddings) < 2:
        log("Phase 1: Not enough mergeable memories")
        return stats

    max_clusters = SURGE_MAX_CALLS if is_surge else NORMAL_MAX_CALLS

    # Tier 1A: Tight dedup
    log(f"Phase 1A: Tight dedup (threshold={TIGHT_THRESHOLD})...")
    ...
    if len(mergeable_embeddings) >= 2:
        log(f"Phase 1B: Topic merge (threshold={TOPIC_THRESHOLD})...")
        ids, sim_matrix = cosine_similarity_matrix(mergeable_embeddings)
    ...
    log(f"Phase 1 done: {stats['tight_found']}+{stats['topic_found']} clusters found, "
...
def phase_weave(conn, all_embeddings, mem_by_id, is_surge, execute=False):
    ...
    if len(all_embeddings) < 4:
        log("Phase 2: Not enough memories for cross-category weaving")
        return stats

    log("Phase 2: Thematic weaving (cross-category connections)...")

    # Filter out dream-generated memories. only weave original content
    original_ids = {
        mid for mid in all_embeddings
        if not _is_dream_generated(mem_by_id.get(mid, {}))
    }
    log(f"  {len(original_ids)} original memories (skipping {len(all_embeddings) - len(original_ids)} dream-generated)")
    ...
    for mid_a, mid_b, cos in cross_pairs:
        ...
        if link_type:
            ...
            if insight:
                tag_str = f"synthesized,dream-cycle,bridge,src:dream-{time.strftime('%Y-%m-%d')}"
                conn.execute(
    ...
    log(f"Phase 2 done: {stats['pairs_evaluated']} evaluated, "
...
def phase_contradict(conn, mergeable_embeddings, mem_by_id, is_surge, execute=False):
    ...
    if len(mergeable_embeddings) < 2:
        log("Phase 3: Not enough memories for contradiction scan")
        return stats

    log("Phase 3: Contradiction scan...")
    ...
    log(f"Phase 3 done: {stats['candidates']} evaluated. "
...
def phase_synthesize(conn, all_embeddings, mem_by_id, execute=False):
    ...
    if len(active) < 10:
        log("Phase 4: Not enough active memories for synthesis")
        return stats

    log("Phase 4: Insight synthesis (dream generation)...")

    # Select dream packet. diverse, high-signal memories
    packet_ids = _select_dream_packet(conn, active, all_embeddings)
    stats["packet_size"] = len(packet_ids)
    log(f"  Dream packet: {len(packet_ids)} memories across "
    ...
    memories_text = "\n\n".join(parts)
    prompt = (
        f"Dream packet: {len(packet_ids)} memories from "
    ...
    # Parse and store individual insights
    insights = _parse_insights(result)
    tag_str = f"synthesized,dream-cycle,src:dream-{time.strftime('%Y-%m-%d')}"
    ...
    if insights:
        # Store dream metadata
        source_ids_json = json.dumps(packet_ids)
        conn.execute(
            "INSERT INTO dream_insights (source_ids, insight_type, dream_run_id) "
        ...
        for row in conn.execute(
            ...
            (f"%src:dream-{time.strftime('%Y-%m-%d')}%",)
        ).fetchall():
            try:
                text = prep_memory_text("personal", row[1], row[2], mem_type="learning")
                emb = fastembed_embed([text])
                if emb and emb[0]:
                    thash = text_hash(text)
    ...
    log(f"Phase 4 done: {stats['insights_generated']} insights generated "
        f"from {stats['packet_size']} memories")
    return stats


def _select_dream_packet(conn, active_memories, embeddings):
    ...
    for mid, m in active_memories.items():
        # Skip dream-generated memories. only synthesize from originals
        if _is_dream_generated(m):
            continue
    ...
    selected = []
    while len(selected) < DREAM_PACKET_SIZE:
        added = False
        for proj in sorted(by_project.keys()):
            if by_project[proj] and len(selected) < DREAM_PACKET_SIZE:
