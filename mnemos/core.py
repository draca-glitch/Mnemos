...
from .storage.base import MnemosStore, Memory, SearchResult
from .storage.sqlite_store import SQLiteStore
from .embed import embed, prep_memory_text
from .rerank import rerank, rrf_merge
from .query import clean_fts_query, fts_dedup
...
class Mnemos:
    ...
    def _unified_dedup(self, content, project, tags, mem_type="", layer=""):
        ...
        # 2) CML subject conflict (same subject prefix in same project)
        cml_type, subject = _extract_cml_subject(content)
        # (Skipped here for brevity; SQLite store would scan content LIKE)
    ...
    def search(self, query, project=None, subcategory=None, layer=None,
        ...
        When `expand_merged=True`, results that were created by the dream
    ...
    def consolidate(self, execute: bool = False, phases: set = None,
                    surge: bool = False, project: str = None) -> dict:
        """Run the dream cycle consolidation. Requires LLM for phases 1-4.

        Phase 5 (Bookkeeping) always runs without LLM. See
        mnemos.consolidation.llm for environment variable configuration.
        """
        from .consolidation import run_dream_cycle
